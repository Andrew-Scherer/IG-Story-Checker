...
async def process_batch_async(batch_id: str, worker_pool: WorkerPool):
    ...
    try:
        with app.app_context():
            # Log batch lookup
            app.logger.info(f"=== Starting Batch {batch_id} ===")
            app.logger.info("1. Looking up batch in database...")
            batch = db.session.get(Batch, batch_id)
            if not batch:
                app.logger.error(f'Batch {batch_id} not found')
                BatchLogService.create_log(batch_id, 'ERROR', f'Batch {batch_id} not found')
                return

            # Log batch start
            app.logger.info(f"2. Found batch with {batch.total_profiles} profiles")
            ...
            app.logger.info("3. Updating batch status to in_progress...")
            ...
            app.logger.info("4. Beginning profile processing...")
            ...
            for batch_profile in batch.profiles:
                ...
                app.logger.info("5. Requesting worker from pool...")
                ...
                while not worker and retry_count < MAX_WORKER_RETRIES:
                    ...
                    if not worker:
                        ...
                        app.logger.info(f'No workers available, waiting {wait_time}s before retry {retry_count}/{MAX_WORKER_RETRIES}...')
                        BatchLogService.create_log(
                            batch_id,
                            'WORKER_WAIT',
                ...
                if not worker:
                    app.logger.error('Failed to get worker after max retries')
                    BatchLogService.create_log(
                        batch_id,
                        'WORKER_UNAVAILABLE',
                ...
                app.logger.info(f'6. Got worker with proxy {worker.proxy_session.proxy_url_safe}')
                BatchLogService.create_log(
                    batch_id,
                    'WORKER_ASSIGNED',
                ...
                try:
                    # Check story
                    app.logger.info(f'7. Checking story for {batch_profile.profile.username}')
                    BatchLogService.create_log(
                        batch_id,
                        'STORY_CHECK_START',
                        ...
                        try:
                            ...
                            if "Rate limited" in str(e):
                                ...
                                if retry_count < MAX_RETRIES:
                                    app.logger.info(f'Rate limited, waiting before retry {retry_count} for {batch_profile.profile.username}')
                    ...
                    app.logger.info(f'8. Releasing worker with proxy {worker.proxy_session.proxy_url_safe}')
            ...
            app.logger.info("9. Batch processing complete")

            # Mark current batch as completed using QueueManager
            app.logger.info(f'Marking batch {batch.id} as completed')
            with app.app_context():
            ...
            app.logger.info(f'Unregistered batch {batch_id} from worker pool')

    except Exception as e:
        app.logger.error(f'Error processing batch {batch_id}: {e}', exc_info=True)
...
def process_batch(batch_id: str):
    ...
    app.logger.info(f"=== Starting process_batch for batch_id: {batch_id} ===")
    ...
    try:
        with app.app_context():
            # Get worker pool
            app.logger.info("1. Getting worker pool...")
            worker_pool = getattr(app, 'worker_pool', None)
            app.logger.info(f"2. Worker pool obtained: {worker_pool is not None}")
            if not worker_pool:
                app.logger.error('No worker pool available')
                BatchLogService.create_log(batch_id, 'ERROR', 'No worker pool available')
                return

            # Verify batch exists before logging
            app.logger.info("3. Verifying batch exists...")
            batch = db.session.get(Batch, batch_id)
            if not batch:
                app.logger.error(f'Batch {batch_id} not found')
                BatchLogService.create_log(batch_id, 'ERROR', f'Batch {batch_id} not found')
            ...
            app.logger.info(f"4. Running process_batch_async for batch_id: {batch_id}")
            try:
                loop.run_until_complete(process_batch_async(batch_id, worker_pool))
            except Exception as e:
                app.logger.error(f'Error in process_batch_async: {e}', exc_info=True)
        ...
        with app.app_context():
            ...
            try:
                ...
                app.logger.error(f'Error resetting batch status: {e}', exc_info=True)
    finally:
        loop.close()
        with app.app_context():
            app.logger.info(f"=== Finished process_batch for batch_id: {batch_id} ===")
